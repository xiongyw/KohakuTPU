...
They are **`rom_style = "block"` and SYNCHRONOUS**, and both halves of that are
load-bearing. This section used to argue for `"distributed"` on two grounds and
**both were wrong**; the tables moved to block RAM and it saved **3,575 LUT per
core** at 24 tiles, with `vec_alu`'s measured error unchanged to three decimals.

- *"A BRAM adds a cycle and breaks the stage arithmetic."* The address was
  already registered (`u_d_ix`, D=1), so a synchronous ROM is a **register
  move**: delete `u_d_ix`, feed `tab_idx` raw, and `c0/c1/c2` still land on
  cycle 3.
- *"Every lane needs its own copy at a different index, which no block RAM port
  count can serve."* Each lane simply has its own ROM. Port count never entered
  into it.

The primitive is still **named, never inferred** — that rule is intact, and with
a clocked ROM the read latency is stated by the RTL rather than by a heuristic,
which is what the rule is actually protecting.

**The trap, if this is ever regenerated by hand:** the case body must use
non-blocking assignments. `vec_alu`'s `u_d_c1` registers `c1` on the same edge,
so blocking assignments in the clocked block are a scheduler race that corrupts
the coefficients while looking exactly like a pipeline-alignment bug. See
`.plan/measurements/vector-lut.md`.
...
### 7.3a Sixteen ALUs: a chain OR a tree, never both

**The hard constraint on every fused reduction.** TREE spends its sixteen ALUs
as 8 leaves + 7 combine nodes + 1 accumulator — *the tree physically is the
other eight ALUs*. An 8-wide × 2-deep chain also spends all sixteen. So a
reduction fused with a **two-stage** elementwise computation does not fit, and
no amount of wiring makes it fit.

That is the whole reason `VEXPSUM` (`VRED` kind 5) is unary. `exp2(a)` is one
stage, so it sits in the leaf and the tree survives; `exp2(a - m)` is two, and
it would need a **fused single-stage leaf op** inside `vec_alu` — measured at
~800 LUT per core for the bias alone, because the round and the negate adders
are parallel by design (§4.4) and both need the extra term.

The corollary is the useful one: **any unary elementwise op can be fused with a
reduction for almost nothing.** Writing the leaves back costs 221 LUT per core,
because slice *s* comes from leaf *s mod 8* and below 8 that is the wire FLAT
already selects — only the top eight slices need a new mux input.
...
3R1W per lane, built as **three duplicated copies: b and c in block RAM, a in
LUTRAM.** Port a is the exception because it is the only one whose consumer
cannot afford a block RAM's clock-to-out — it feeds `ls_rdata` and thence the
store converters, and `CLKARDCLK -> DO` (~1.5 ns on -2L) in series with a
16-lane E8→FP16 normalise measured **286.0 MHz**, below the 300 floor. b and c
feed ALU operands, which get a whole cycle. Registering between the memory and
the converters would cost a cycle on every `VST` beat, and the machine is
drain-bound. The block-RAM sizing below applies to b and c:
`128 deep × 24 bit` is 3 Kb, a rounding error against a block RAM, but as
distributed RAM the same three copies measured **168 LUT per lane, 2,688 per
core** — spent on storage, in a design where LUT binds and BRAM sits near idle.
48 RAMB18 per core is 24 of an SLR's 672 tiles. URAM is the wrong end of the
trade: 4096 × 72 is its smallest shape, so 48 ports would take 15% of an SLR's
URAM to hold 49 Kb. See `.plan/measurements/vector-lut.md`.
...
## 11a. The shrink: measured, and what it cost

`.plan/measurements/vector-lut.md` has the per-step tables. The conclusions:
...
longer in `vec_core` at all — it is `vec_alu`'s own `s1_b_reg[16] -> u_d_can`,
so the core now sits at the ALU floor. `mm_mesh` measures 328.8 MHz.

Per change, all measured on `vec_lanes` unless noted:

| | LUT |
|---|---|
| operand network (phase window + constant indices) | **−3,404** |
| coefficient ROMs to block RAM | **−3,575** |
| register file to block RAM | −3,352, of which **+1,129 came back** (below) |
| predicate write-back | −1,987 |
| stage-0 narrowing | −1,256 |
| write crossbar | −1,089 |
| lane rotate (`vec_core`, VSHUF) | −565 |
| `VEXPSUM` leaf write-back | +249, but **+42 in `vec_cu`** |

**BRAM is now a counted resource**: 44 tiles per core, 176 for four.

### Three things worth carrying forward

**A runtime index over a set the mode already fixes.** Four of these, worth
7,745 LUT together. `wid`/`dep` are functions of `mode`, so `base + phase*wid +
group` is a per-mode constant once indexed by the right thing; written as a
computed index it synthesises as a 16:1 crossbar, written as three literal arms
it is a 3:1. The predicate write-back was CLAUDE.md's variable-part-select-write
trap (`matmul-impl.md` §3.1) in miniature and was nearly skipped as too small.

**Blocking assignments in a block converted from `always @(*)` to
`always @(posedge clk)`.** This cost two wasted runs on `vec_tables`. The ROM's
`c1` is registered by `u_d_c1` on the same edge, so blocking assignments are a
scheduler race — and it corrupted *both* pipeline alignments, including the one
that had to be exactly equivalent. **When the variant that must be equivalent is
not, stop hypothesising about timing and look for a race.**

**Moving storage to block RAM moves its clock-to-out onto every consumer's
path, and PORT GRANULARITY is the unit that matters, not the module.** A
RAMB18's `CLKARDCLK -> DO` is ~1.5 ns on -2L. Of the register file's three read
ports, b and c feed ALU operands and have a whole cycle; a feeds the store
converters and did not — `vec_cu` fell to **286.0 MHz**. The load side had
already hit this and left `cv_src` behind at **286.9 MHz**, the same number one
direction earlier. A whole-module `PRIM` parameter hid that only one of three
consumers could not afford it; splitting `PRIM_A` out kept two thirds of the
area win for one eighth of the BRAM. Note also that this is the *placement-
robust* kind of fix: a primitive's clock-to-out is fixed silicon, so removing it
pays in full after place-and-route, where a logic-depth reduction is swamped by
routing.

### A test never seen to fail is not a test

`VRED` kinds 6/7 had no coverage anywhere, and the VL mask they reduce under is
the kind of thing a *passing* test can miss entirely: a uniform predicate gives
the same answer whether the mask is right or stuck at all-ones. The test added
in `vec_cu_tb` §5 splits the predicate at element 64 and reduces under vl=64, so
a stale mask is wrong in both directions — and **it was then verified by forcing
`vlmask` to all-ones and watching it fail** (`got 3c00 want 0`).

That negative control is what makes the registered `vlmask` an empirical result
rather than an argument. Do it for anything whose failure mode is "quietly
returns a plausible value": write the test, then break the thing on purpose and
confirm the test notices.

### One coupling the next person must know about

**`vec_lanes` is no longer independently correct for inputs its bench never
generates.** The stage-0 narrowing relies on `vec_core` raising `F_VSRC` for any
`SRC_V` past chain slot 0, so eight of sixteen ALUs have no register-file path
in D2/D4. An illegal stage-*t*>0 `SRC_V` reads a wrong *lane* rather than a
wrong *element*. Both are wrong and neither is reachable through the sequencer,
but the module can no longer be reused on its own without re-checking that.
Reverting is the `g_v4`/`g_v2`/`g_v1` generate in `vec_lanes.v`, worth 1,256 LUT.
...
**The assembled core is a different measurement, and a harder one.** One lane at
324.8 MHz says nothing about `vec_lanes` (16 lanes, register file, crossbar) or
`vec_cu` (sequencer, L1, AGU, NoC endpoint) — those started at 305.1 and
**229.3 MHz**, and the paths that bound them were all control reaching a
datapath, never the arithmetic. `docs/memory-mover/arch.md` §10.2–10.3 has every
fix and every number; the two worth carrying back here are that **a module whose
control inputs are top-level ports is flattered by OOC synthesis** (`vec_lanes`
met 300 only because `mode` had an ideal driver, and inside `vec_cu` it is a
register in `vec_core`), and that **nothing may sit in series with the sixteen
parallel FP16 normalises** — not a BRAM's clock-to-out, not a 2:1 mux.
