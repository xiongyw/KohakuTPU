// ktpu_mesh_2x2_1port -- GENERATED by scripts/py/gen_mesh.py. Do not edit by hand.
//
//   xxx vec vec xxx
//   mag mat nul nul
//   nul mat nul nul
//   xxx vec vec xxx
//
// 2x2 routers, 1 MAG port(s), 2 cluster(s), 4 vector core(s).
...
//
// MAG presents MEM_PORTS+2 = 3 AXI masters -- one per memory port, plus the
...
module ktpu_mesh_2x2_1port #(
    ...
    (* X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF S_AXI_MEM:S_AXI_CTRL:M_AXI_MEM0:M_AXI_UPLOAD:M_AXI_MOVER, ASSOCIATED_RESET axi_aresetn" *)
    input  wire axi_aclk,
    ...
    assign M_AXI_UPLOAD_awid = mm_awid[1*IDW +: IDW];
    assign M_AXI_UPLOAD_awaddr = mm_awaddr[1*AW +: AW];
    assign M_AXI_UPLOAD_awlen = mm_awlen[1*8 +: 8];
    assign M_AXI_UPLOAD_awsize = mm_awsize[1*3 +: 3];
    assign M_AXI_UPLOAD_awburst = mm_awburst[1*2 +: 2];
    assign M_AXI_UPLOAD_awvalid = mm_awvalid[1];
    assign mm_awready[1] = M_AXI_UPLOAD_awready;
    assign M_AXI_UPLOAD_wdata = mm_wdata[1*DW +: DW];
    assign M_AXI_UPLOAD_wstrb = mm_wstrb[1*DW/8 +: DW/8];
    assign M_AXI_UPLOAD_wlast = mm_wlast[1];
    assign M_AXI_UPLOAD_wvalid = mm_wvalid[1];
    assign mm_wready[1] = M_AXI_UPLOAD_wready;
    assign mm_bid[1*IDW +: IDW] = M_AXI_UPLOAD_bid;
    assign mm_bresp[1*2 +: 2] = M_AXI_UPLOAD_bresp;
    assign mm_bvalid[1] = M_AXI_UPLOAD_bvalid;
    assign M_AXI_UPLOAD_bready = mm_bready[1];
    assign M_AXI_UPLOAD_arid = mm_arid[1*IDW +: IDW];
    assign M_AXI_UPLOAD_araddr = mm_araddr[1*AW +: AW];
    assign M_AXI_UPLOAD_arlen = mm_arlen[1*8 +: 8];
    assign M_AXI_UPLOAD_arsize = mm_arsize[1*3 +: 3];
    assign M_AXI_UPLOAD_arburst = mm_arburst[1*2 +: 2];
    assign M_AXI_UPLOAD_arvalid = mm_arvalid[1];
    assign mm_arready[1] = M_AXI_UPLOAD_arready;
    assign mm_rid[1*IDW +: IDW] = M_AXI_UPLOAD_rid;
    assign mm_rdata[1*DW +: DW] = M_AXI_UPLOAD_rdata;
    assign mm_rresp[1*2 +: 2] = M_AXI_UPLOAD_rresp;
    assign mm_rlast[1] = M_AXI_UPLOAD_rlast;
    assign mm_rvalid[1] = M_AXI_UPLOAD_rvalid;
    assign M_AXI_UPLOAD_rready = mm_rready[1];
    assign M_AXI_MOVER_awid = mm_awid[2*IDW +: IDW];
    assign M_AXI_MOVER_awaddr = mm_awaddr[2*AW +: AW];
    assign M_AXI_MOVER_awlen = mm_awlen[2*8 +: 8];
    assign M_AXI_MOVER_awsize = mm_awsize[2*3 +: 3];
    assign M_AXI_MOVER_awburst = mm_awburst[2*2 +: 2];
    assign M_AXI_MOVER_awvalid = mm_awvalid[2];
    assign mm_awready[2] = M_AXI_MOVER_awready;
    assign M_AXI_MOVER_wdata = mm_wdata[2*DW +: DW];
    assign M_AXI_MOVER_wstrb = mm_wstrb[2*DW/8 +: DW/8];
    assign M_AXI_MOVER_wlast = mm_wlast[2];
    assign M_AXI_MOVER_wvalid = mm_wvalid[2];
    assign mm_wready[2] = M_AXI_MOVER_wready;
    assign mm_bid[2*IDW +: IDW] = M_AXI_MOVER_bid;
    assign mm_bresp[2*2 +: 2] = M_AXI_MOVER_bresp;
    assign mm_bvalid[2] = M_AXI_MOVER_bvalid;
    assign M_AXI_MOVER_bready = mm_bready[2];
    assign M_AXI_MOVER_arid = mm_arid[2*IDW +: IDW];
    assign M_AXI_MOVER_araddr = mm_araddr[2*AW +: AW];
    assign M_AXI_MOVER_arlen = mm_arlen[2*8 +: 8];
    assign M_AXI_MOVER_arsize = mm_arsize[2*3 +: 3];
    assign M_AXI_MOVER_arburst = mm_arburst[2*2 +: 2];
    assign M_AXI_MOVER_arvalid = mm_arvalid[2];
    assign mm_arready[2] = M_AXI_MOVER_arready;
    assign mm_rid[2*IDW +: IDW] = M_AXI_MOVER_rid;
    assign mm_rdata[2*DW +: DW] = M_AXI_MOVER_rdata;
    assign mm_rresp[2*2 +: 2] = M_AXI_MOVER_rresp;
    assign mm_rlast[2] = M_AXI_MOVER_rlast;
    assign mm_rvalid[2] = M_AXI_MOVER_rvalid;
    assign M_AXI_MOVER_rready = mm_rready[2];
    ...
    mag #(.FLIT_WIDTH(FW), .POS_WIDTH(PW), .DATA_W(DW), .ADDR_W(AW),
          .ID_W(IDW), .MEM_PORTS(1), .MEM_X(0), .MEM_Y(1),
        ...
        .mem_in_data({l000_h0_1_rd}), .mem_in_valid(mag_i_v), .mem_in_busy(mag_i_b),
    ...
    assign l009_h0_2_fd = {FW{1'b0}}; assign l009_h0_2_fv = 1'b0; assign l009_h0_2_rb = 1'b0;
