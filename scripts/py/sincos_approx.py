...
plt.plot(x.numpy(), cos_target.numpy(), label="True cos(x)")
...
# the cosine plot shifts by +0.5*pi; the sine one does not
plt.subplot(1, 2, 2)
with torch.no_grad():
    y_pred_sin = x - x**3 * 1 / 6 + x**5 * 1 / 120  # - x**7 * 1/5040 #+ x**9 * 1/362880
plt.plot(x.numpy(), sin_target.numpy(), label="True sin(x)")
